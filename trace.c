...
void print_ethernet_header(ethernet_frame ether_frame, uint32_t packet_num,
                           uint32_t frame_len);
void processes_ether_type_headers(uint16_t type, const u_char *pkt_data,
                                  uint32_t data_size);
void parse_arp_header(const u_char *pkt_data);
void print_ip_header(const u_char *pkt_data);
void print_arp_header(const u_char *pkt_data);
void print_tcp_header(const u_char *pkt_data, ipv4_header *);
void print_udp_header(const u_char *pkt_data);
void print_icmp_header(const u_char *pkt_data);
  ...
  */
  ...
  while (pcap_next_ex(pcap_file, &pkt_header, &pkt_data) == 1) {
    ...
    if (pcap_datalink(pcap_file) != DLT_EN10MB) {
      ...
      processes_ether_type_headers(ntohs(ether_frame.type), pkt_data + 13,
                                   pkt_header->len - 14);
    }
  }
  return 0;
...
void print_ethernet_header(ethernet_frame ether_frame, uint32_t packet_num,
  ...
  printf("\t\tDest MAC: ");
  char *mac_addr =
      ether_ntoa((const struct ether_addr *)ether_frame.dest_mac_addr);
  printf("%s", mac_addr);
  printf("\n");
  printf("\t\tSource MAC: ");
...
void print_arp_header(const u_char *pkt_data) {
  ...
  char *ip_addr;
  char *mac_addr =
      ether_ntoa((const struct ether_addr *)arp_head.sender_mac_addr);
  printf("\t\tSender MAC: %s\n", mac_addr);
  printf("\t\tSender IP: ");
...
void print_ip_header(const u_char *pkt_data) {
  ...
  pkt_data +=
      1 + ip_header.header_length *
              4; // incremeintg hte data pointer to print out the next header
  ...
  (in_cksum((unsigned short *)(&ip_header), ip_header.header_length * 4))
      ? printf("Incorrect ")
      : printf("Correct ");
...
void print_tcp_header(const u_char *pkt_data, ipv4_header *ip_header) {
  printf("\n");
  ...
  ip_sudo_head.tcp_length =
      htons(ntohs(ip_header->total_length) - (ip_header->header_length * 4));
  ...
  memcpy(check_sum_header + sizeof(ip_sudo_head), pkt_data,
         ntohs(ip_sudo_head.tcp_length));
  printf("\t\tChecksum: ");
